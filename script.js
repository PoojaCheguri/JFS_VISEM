function showInventor() {
    ...
    if (inventor === "") {
        ...
        img.classList.remove("shadow-effect");
    ...
    // Apply square + shadow only for Gosling
    if (inventor === "gosling") {
        img.classList.remove("circle-image");
        img.classList.add("square-image", "shadow-effect");
    } else {
        img.classList.remove("square-image", "shadow-effect");
...
// Blink shadow on click
function blinkShadow() {
    const img = document.getElementById("inventorImage");
    img.classList.add("blink");

    // Remove the class after animation completes to allow future clicks
    setTimeout(() => {
        img.classList.remove("blink");
    }, 1800); // 0.6s * 3 times = 1.8s
}
